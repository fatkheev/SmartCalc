...
int main(int argc, char *argv[]) {
  ...
  auto calculatorButtonClicked = [lineEdit, xValueLineEdit,
                                  &closeParenthesisIfNeeded](QPushButton *btn) {
    if (btn->text() == "C") {
      lineEdit->clear();
    } else if (btn->text() == "=") {
  ...
  upperBox->setStyleSheet(
      "QGroupBox { border: 2px solid gray; border-radius: 9px; }");
  lowerBox->setStyleSheet(
      "QGroupBox { border: 2px solid gray; border-radius: 9px; }");
  ...
  QStringList lowerButtons = {"7", "8", "9", "+", "4", "5", "6", "-", "1",
                              "2", "3", "*", "0", ".", "^", "/", "="};
  ...
  for (int i = 0; i < upperButtons.size(); ++i) {
    ...
    upperLayout->addWidget(btnC, 0, 0, 1,
                           4);  // Занимает первую строку и все 4 столбца
    QObject::connect(
        btnC, &QPushButton::clicked,
        [btnC, &calculatorButtonClicked]() { calculatorButtonClicked(btnC); });

    for (int i = 1; i < upperButtons.size();
         ++i) {  // Стартуем с 1, так как "C" уже добавлена
      QPushButton *btn = new QPushButton(upperButtons[i], calculatorWidget);
      btn->setStyleSheet(smallerButtonStyle);
      upperLayout->addWidget(
          btn, 1, i - 1);  // Изменим индексы для отступа под кнопкой "C"
      QObject::connect(
          btn, &QPushButton::clicked,
          [btn, &calculatorButtonClicked]() { calculatorButtonClicked(btn); });
    }
  }
  ...
  for (int i = 0; i < lowerButtons.size(); ++i) {
    ...
    if (lowerButtons[i] == "=") {
      ...
      lowerLayout->addWidget(btn, row, col, 1,
                             4);  // Занимает 1 строку и 4 столбца
  ...
  trigBox->setStyleSheet(
      "QGroupBox { border: 2px solid gray; border-radius: 9px; }");
  trigLayout->setContentsMargins(10, 30, 10, 10);
  QStringList trigButtons = {"sin", "cos", "tan", "asin", "acos", "atan"};
  ...
  QObject::connect(plotButton, &QPushButton::clicked, [&]() {
    ...
    double xMin = xMinLineEdit->text().isEmpty()
                      ? -M_PI
                      : xMinLineEdit->text().toDouble();
    double xMax =
        xMaxLineEdit->text().isEmpty() ? M_PI : xMaxLineEdit->text().toDouble();
    double yMin =
        yMinLineEdit->text().isEmpty() ? -2 : yMinLineEdit->text().toDouble();
    double yMax =
        yMaxLineEdit->text().isEmpty() ? 2 : yMaxLineEdit->text().toDouble();

    plot->clearGraphs();
    QString baseExpr =
        lineEdit->text();  // Используем lineEdit вместо graphExpression
  ...
  QPushButton *toggleCreditCalculatorButton =
      new QPushButton("Кредитный калькулятор", mainWidget);
  ...
  QObject::connect(&creditCalculator,
                   &CreditCalculatorWindow::switchToMainCalculator, [&]() {

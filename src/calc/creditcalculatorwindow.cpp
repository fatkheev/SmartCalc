...
void CreditCalculatorWindow::calculate() {
    ...
    if (paymentType == "Аннуитетный") {
        ...
        double maxPayment = (loanAmount / months) + loanAmount / months * monthlyRate;

        resultMonthlyPayment->setText(QString::number(minPayment, 'f', 2) + " ... " + QString::number(maxPayment, 'f', 2));

        double totalInterestPaid = (minPayment + maxPayment) * months / 2 - loanAmount;
        resultAccruedInterest->setText(QString::number(totalInterestPaid, 'f', 2));
        resultTotalPayment->setText(QString::number(loanAmount + totalInterestPaid, 'f', 2));
